...
export async function POST(req: Request) {
    try {
        ...
        let {examType, subject, chapter, subtopic, difficulty, questionType} = body
        ...
        if (subtopic) {
            query.subtopicName = {$regex: new RegExp(subtopic, 'i')}
        }
        ...
        const questions = await Question.find(query).sort({questionNo: 1}).lean().exec()
