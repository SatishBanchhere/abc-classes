...
import { getJeePageData, getHomePageData } from "@/lib/data-fetcher"
import React, {useEffect, useState} from "react";
import Link from "next/link"

export default async function JeePage() {
  const homeData = await getHomePageData();
const jeeData = await getJeePageData();
  // const [jeeData, setJeeData] = useState(null);
  ...
  //       // @ts-ignore
  //       setHomeData(data);
  //       const data2 = await getJeePageData();
  //       // @ts-ignore
  //       setJeeData(data2);
  //     } catch (error) {
      ...
      <div className="flex min-h-screen w-full flex-col relative overflow-hidden">
        ...
        <section className="relative py-24 px-6 bg-gradient-to-br from-slate-50 via-blue-50/50 to-indigo-50/30">
          ...
          <div className="container mx-auto text-center relative z-10">
            <div className="inline-flex items-center gap-2 bg-gradient-to-r from-[#1a73e8]/10 to-[#4285f4]/10 border border-[#1a73e8]/20 rounded-full px-4 py-2 mb-8">
              <Sparkles className="h-4 w-4 text-[#1a73e8]" />
              <span className="text-sm font-medium text-[#1a73e8]">{jeeData.pageSubtitle}</span>
            </div>
              ...
              <div>
                <Card className="border-0 shadow-xl hover:shadow-2xl transition-all duration-300 bg-gradient-to-br from-white to-slate-50/50 h-full">
                  <CardHeader className="pb-6">
                    ...
                    <CardTitle className="text-3xl font-bold text-slate-900 mb-4">{jeeData.physicsTitle}</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-6">
                    <CardDescription className="text-base text-slate-600 leading-relaxed">
                      {jeeData.physicsDescription}
                    </CardDescription>

                    <div className="space-y-3">
                      {jeeData.physicsFeatures.map((feature, index) => (
              ...
              <div>
                <Card className="border-0 shadow-xl hover:shadow-2xl transition-all duration-300 bg-gradient-to-br from-white to-slate-50/50 h-full">
                  <CardHeader className="pb-6">
                    ...
                    <CardTitle className="text-3xl font-bold text-slate-900 mb-4">{jeeData.chemistryTitle}</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-6">
                    <CardDescription className="text-base text-slate-600 leading-relaxed">
                      {jeeData.chemistryDescription}
                    </CardDescription>

                    <div className="space-y-3">
                      {jeeData.chemistryFeatures.map((feature, index) => (
                        <div key={index} className="flex items-start gap-3">
        ...
        <section className="py-24 px-6 bg-gradient-to-br from-slate-50 to-blue-50/30 relative z-10">
          <div className="container mx-auto">
            <div className="max-w-4xl mx-auto text-center">
              ...
              <p className="text-lg text-slate-600 leading-relaxed mb-8">{jeeData.additionalInfo}</p>
            </div>
          </div>
        </section>
        ...
        <section className="py-24 px-6 bg-gradient-to-br from-[#1a73e8] to-[#4285f4] text-white relative z-10 overflow-hidden">
          ...
          <div className="container mx-auto text-center relative z-10">
            <h2 className="text-4xl md:text-5xl font-bold mb-6">Ready to Excel in JEE?</h2>
            <p className="text-xl mb-8 opacity-90 max-w-2xl mx-auto">{jeeData.ctaText}</p>
            <Link
                href="/auth/register"
                className="inline-flex items-center justify-center text-lg px-10 py-4 bg-white text-[#1a73e8] hover:bg-slate-100 shadow-xl font-medium rounded-md transition-colors duration-300"
            >
              Enroll Now
            </Link>
          </div>
